...
public class Level8ManagerEQ : MonoBehaviour
{
    public static Level8ManagerEQ instance;
    ...
    public GameObject firstAidUITool; // BAGONG DAGDAG PARA SA NPC QUEST

    [Header("NPC Quest (Drag & Drop)")]
    public TMP_Text timeBonusTextUI; 
    public float bonusTimeReward = 10f;

    [Header("Item Pickup Animation")]
    public Image presentationImage; 
    public Sprite keySprite;        
    public Sprite extinguisherSprite; 
    public Sprite firstAidSprite; // BAGONG DAGDAG
    ...
    IEnumerator IntroSequenceRoutine()
    {
        ...
        StarManager.Instance.PauseTimer();
    ...
    // ==========================================
    // --- LEVEL CORE MECHANICS (MAZE & TOOLS) ---
    // ==========================================
    ...
    // BAGONG DAGDAG: Kapag nadampot ni Jobert ang First Aid Kit sa daan
    public void PickUpFirstAid()
    {
        if (!isGameActive) return;
        if (AudioManager.instance != null) AudioManager.instance.PlaySFX(AudioManager.instance.clickSound);
        StartCoroutine(ItemPickupAnimation(firstAidSprite, firstAidUITool));
    }
    ...
    IEnumerator ItemPickupAnimation(Sprite itemSprite, GameObject targetUITool)
    {
        ...
        Vector3 targetPos = targetUITool.GetComponent<RectTransform>().position; 
        ...
        while (elapsed < duration)
        {
            ...
            rect.localScale = Vector3.Lerp(Vector3.one * 1.5f, Vector3.one, t); 
            yield return null;
        }
    ...
    public void UseTool(string toolNameUsed)
    {
        ...
        if (currentObstacleNearPlayer != null)
        {
            ...
            if (obsData != null && obsData.correctTool == toolNameUsed)
            {
                ...
                // ==========================================
                // BAGONG LOGIC PARA SA FIRST AID (NPC BONUS)
                // ==========================================
                if (toolNameUsed == "FirstAidKit")
                {
                    if (StarManager.Instance != null) StarManager.Instance.AddBonusTime(bonusTimeReward);
                    if (timeBonusTextUI != null) StartCoroutine(AnimateBonusText());
                }
    ...
    {
        timeBonusTextUI.gameObject.SetActive(true);
        timeBonusTextUI.text = "+" + bonusTimeReward;
        Vector3 startPos = timeBonusTextUI.rectTransform.localPosition;
        Color c = timeBonusTextUI.color; c.a = 1f; timeBonusTextUI.color = c;
        
        float timer = 0f;
        while (timer < fadeDuration)
        {
            timer += Time.deltaTime;
            timeBonusTextUI.rectTransform.localPosition += Vector3.up * fallSpeed * Time.deltaTime; 
            c.a = Mathf.Lerp(1f, 0f, timer / fadeDuration);
            timeBonusTextUI.color = c;
            yield return null;
        }
        
        timeBonusTextUI.gameObject.SetActive(false);
        timeBonusTextUI.rectTransform.localPosition = startPos; 
    }
